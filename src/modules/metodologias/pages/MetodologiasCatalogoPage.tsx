  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-2">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Catálogo canônico de ativos</h2>
          <p className="text-slate-500 text-sm">
            Busca e leitura rápida por filtros sem acoplar detalhe completo na mesma página.
          </p>
        </div>
        <span className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Frente separada</span>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-2">
          <input
            ...
            className="xl:col-span-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-cyan-300"
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-2">
          <select value={filtros.possui_blocos_canonicos} onChange={(event) => atualizarFiltro('possui_blocos_canonicos', event.target.value as CatalogoFiltrosAvancados['possui_blocos_canonicos'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={filtros.possui_versao_vigente} onChange={(event) => atualizarFiltro('possui_versao_vigente', event.target.value as CatalogoFiltrosAvancados['possui_versao_vigente'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={filtros.snapshot_equivalencia} onChange={(event) => atualizarFiltro('snapshot_equivalencia', event.target.value as CatalogoFiltrosAvancados['snapshot_equivalencia'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={filtros.vindo_de_promocao} onChange={(event) => atualizarFiltro('vindo_de_promocao', event.target.value as CatalogoFiltrosAvancados['vindo_de_promocao'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={filtros.origem_rastreavel} onChange={(event) => atualizarFiltro('origem_rastreavel', event.target.value as CatalogoFiltrosAvancados['origem_rastreavel'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={filtros.manutencao_recente} onChange={(event) => atualizarFiltro('manutencao_recente', event.target.value as CatalogoFiltrosAvancados['manutencao_recente'])} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-2">
          <select value={ordenacao} onChange={(event) => setOrdenacao(event.target.value as CatalogoOrdenacao)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <select value={agrupamento} onChange={(event) => setAgrupamento(event.target.value as CatalogoAgrupamento)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800">
          ...
          <div className="xl:col-span-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-xs text-slate-600 flex items-center">
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg border border-slate-200 text-xs font-black uppercase tracking-wide text-slate-600 hover:bg-slate-50"
          >
            Limpar filtros
          </button>
        </div>
        ...
        {exploracao.resumo.possui_filtros_ativos && (
          <div className="flex flex-wrap gap-1.5">
            {exploracao.resumo.filtros_ativos.map((filtro) => (
              <span key={filtro} className="px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wide bg-cyan-50 text-cyan-700">
                {filtro}
              </span>
            ))}
          </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 md:p-5 space-y-4">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-2">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">Mapa de conexões</p>
            <h3 className="text-xl font-black text-slate-900 tracking-tight mt-1">Leitura arquitetural leve do ecossistema</h3>
            <p className="text-sm text-slate-600 mt-1">Visão resumida de conexões entre ativos canônicos no recorte atual do catálogo.</p>
          </div>
          <div className="flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wide">
            <span className="px-2.5 py-1 rounded-md bg-slate-100 text-slate-700">{mapaConexoes.total_ativos} ativos</span>
            <span className="px-2.5 py-1 rounded-md bg-cyan-50 text-cyan-700">{mapaConexoes.total_arestas} conexões</span>
            <span className="px-2.5 py-1 rounded-md bg-violet-50 text-violet-700">{mapaConexoes.total_ativos_conectados} conectados</span>
          </div>
        </div>

        {mapaConexoes.total_arestas === 0 ? (
          <p className="text-sm text-slate-500">Ainda não há conexões registradas para o recorte atual.</p>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <article className="rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-2">
              <p className="text-[11px] font-black uppercase tracking-wide text-slate-600">Arestas (amostra)</p>
                ...
                {mapaConexoes.arestas.map((aresta) => (
                  ...
                  >
                    <p className="text-xs font-semibold text-slate-800 leading-relaxed">
                      {aresta.origem_nome} <span className="text-cyan-700">— {aresta.tipo_relacao_label} →</span> {aresta.destino_nome}
                    </p>
                  </button>
                    );
                  })()
            ...
            <article className="rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-2">
              <p className="text-[11px] font-black uppercase tracking-wide text-slate-600">Ativos mais conectados</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-4">
        <div className="space-y-4">
          {ativosFiltrados.length === 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-6 text-center">
              <h3 className="text-lg font-black text-slate-900">Nenhum ativo encontrado</h3>
              ...
              <button
                type="button"
                onClick={limparExploracao}
                className="mt-4 px-4 py-2 rounded-lg bg-slate-900 text-white text-[11px] font-black uppercase tracking-wide hover:bg-slate-800"
              >
              ...
              <section key={grupo.chave} className="space-y-2">
                {agrupamento !== 'nenhum' && (
                  <div className="flex items-center justify-between px-1">
                    <h3 className="text-sm font-black uppercase tracking-wide text-slate-700">{grupo.label}</h3>
                    <span className="text-[10px] font-black uppercase tracking-wide text-slate-400">{grupo.itens.length} ativo(s)</span>
                  </div>
                )}
        ...
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 h-fit sticky top-4 space-y-3">
          <h3 className="text-sm font-black uppercase tracking-wide text-slate-700">Preview rápido</h3>
          {!ativoPreview ? (
            <p className="text-xs text-slate-500">Sem ativo disponível para preview.</p>
          ) : (
            <>
              <p className="text-base font-black text-slate-900">{ativoPreview.nome}</p>
              <p className="text-xs text-slate-600 leading-relaxed">{ativoPreview.resumo}</p>
