...
const CentralPadroesPage: React.FC = () => {
  ...
  const categoryMap: Array<{ key: GovernanceDocCategory; title: string; docs: GovernanceDoc[]; accent: string }> = [
    ...
    },
    {
      ...
      accent: 'text-gray-800 dark:text-sagb-text'
    },
    {
      key: 'templates',
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-6 overflow-y-auto">
            <article className="prose prose-sm max-w-none text-[12px] text-gray-700">
    ...
    <div className="flex-1 p-10 bg-white dark:bg-sagb-bg text-[12px] text-gray-900 dark:text-sagb-text">
      <header className="mb-10 flex justify-between items-start gap-6">
        ...
        <div className="text-right">
          ...
          <div className="text-lg font-bold text-gray-800">Central de Padrões</div>
      ...
      <section className="bg-gray-50 dark:bg-sagb-bg-2 p-6 rounded-2xl border border-gray-100 dark:border-white/5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h2 className="text-[13px] font-bold text-gray-800 dark:text-sagb-text mb-2">Responsabilidade do Módulo</h2>
            <p className="text-[12px] opacity-80 mb-2">
              <strong>Agente responsável:</strong> {manifest.owner?.displayName || 'A definir'}
            </p>
            <p className="text-[12px] text-gray-500 dark:text-sagb-muted">
              Esta central é o ponto oficial para padronização de design system, stack, nomenclaturas e guardrails técnicos do SagB.
            </p>
          </div>

          <div>
            <h2 className="text-[13px] font-bold text-gray-800 dark:text-sagb-text mb-2">Objetivo Operacional</h2>
            <p className="text-[12px] text-gray-500 dark:text-sagb-muted">
      ...
      <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <article className="bg-white dark:bg-sagb-bg-2 p-4 rounded-xl border border-gray-100 dark:border-white/5">
          <div className="text-[11px] text-gray-500">Documentos</div>
          <div className="text-xl font-black text-gray-800 dark:text-sagb-text">{stats.total}</div>
        </article>
        <article className="bg-white dark:bg-sagb-bg-2 p-4 rounded-xl border border-green-100 dark:border-green-500/20">
          <div className="text-[11px] text-gray-500">🟢 Ativos</div>
          <div className="text-xl font-black text-green-700 dark:text-green-300">{stats.ativos}</div>
        </article>
        <article className="bg-white dark:bg-sagb-bg-2 p-4 rounded-xl border border-yellow-100 dark:border-yellow-500/20">
          <div className="text-[11px] text-gray-500">🟡 Parciais</div>
          <div className="text-xl font-black text-yellow-700 dark:text-yellow-300">{stats.parciais}</div>
        </article>
        <article className="bg-white dark:bg-sagb-bg-2 p-4 rounded-xl border border-red-100 dark:border-red-500/20">
          <div className="text-[11px] text-gray-500">🔴 Pendentes</div>
          <div className="text-xl font-black text-red-700 dark:text-red-300">{stats.pendentes}</div>
        </article>
      </section>

      <section id="governanca-docs" className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
        {categoryMap.map((category) => (
          <article key={category.key} className="bg-white dark:bg-sagb-bg-2 p-5 rounded-2xl border border-gray-100 dark:border-white/5">
            <div className="flex items-center justify-between mb-3">
              <h3 className={`text-[13px] font-bold ${category.accent}`}>{category.title}</h3>
              <span className="text-[11px] text-gray-400">{category.docs.length} docs</span>
            </div>
            ...
            <ul className="space-y-2">
              {category.docs.map((doc) => (
                <li key={doc.id}>
                  <button
                    onClick={() => openDoc(doc)}
                    className="w-full text-left px-3 py-2 rounded-lg border border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-sagb-bg transition"
                  >
                    <div className="flex items-start gap-2 mb-1">
                      <span className={`inline-block w-2.5 h-2.5 rounded-full ${getStatusBadge(doc.status)}`} />
                      <div className="flex-1">
                        <div className="text-[12px] font-semibold text-gray-800 dark:text-sagb-text">{doc.nomeBonito}</div>
                        <div className="text-[10px] text-gray-400 dark:text-sagb-muted mt-0.5">{doc.fileName}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-gray-500 dark:text-sagb-muted">
                      <div className="flex items-center gap-1">
                    ...
                    <p className="text-[11px] text-gray-500 dark:text-sagb-muted mt-1">{doc.description}</p>
                  </button>
                </li>
              ))}
            </ul>
