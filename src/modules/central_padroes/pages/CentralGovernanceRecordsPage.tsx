...
export const CentralGovernanceRecordsPage: React.FC<Props> = ({ table, title, subtitle, icon, recordType, defaultCategory, guidance }) => {
  ...
  return (
    <CentralPageShell title={title} subtitle={subtitle} icon={icon} status="informacao" guidance={guidance}>
      ...
      <CrudModal title={editing ? 'Editar registro' : 'Novo registro'} open={open} onClose={() => setOpen(false)} footer={<button type="button" className="rounded-xl bg-blue-600 px-4 py-2 text-[12px] font-black text-white" onClick={save}>Salvar</button>}>
