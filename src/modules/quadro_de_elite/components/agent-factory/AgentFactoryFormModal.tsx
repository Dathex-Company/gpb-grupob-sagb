...
import { TrashIcon, XIcon } from '../../../../../components/Icon';
import {
  DNA_STATUS_OPTIONS,
  ENTITY_TYPE_OPTIONS,
  LEVEL_OPTIONS,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/70 backdrop-blur-sm p-4 md:p-8 animate-in fade-in duration-200">
      <div className="flex h-full max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[2rem] border border-gray-200/70 dark:border-white/10 bg-white dark:bg-sagb-panel shadow-2xl transition-colors duration-300">
        ...
        <div className="flex-1 space-y-8 overflow-y-auto px-8 py-6 custom-scrollbar">
          ...
          <section className="space-y-4">
            <h3 className="text-[11px] font-black uppercase tracking-[0.16em] text-gray-400 border-b border-gray-100 pb-2">Bloco 2 — Alocação Organizacional</h3>
            <div className="grid grid-cols-1 gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <label className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Nível de Atuação *<HelpTooltip text="Nível hierárquico no ecossistema" /></span>
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Departamento/Unidade<HelpTooltip text="Divisão interna estrutural" /></span>
                  <div className="relative">
                    <input 
                      list="unit-options" 
                      value={form.unitName} 
                      onChange={(e) => onSetFormField('unitName', e.target.value)} 
                      placeholder="Ex: Comercial, Marketing"
                      className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 outline-none focus:border-indigo-300" 
                    />
                ...
                <label className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Área Específica<HelpTooltip text="Setor específico de atuação dentro do departamento" /></span>
                  <div className="relative">
                    <input 
                      list="area-options" 
                      value={form.area} 
                      onChange={(e) => onSetFormField('area', e.target.value)} 
                      placeholder="Ex: Vendas Internas, Performance"
                      className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 outline-none focus:border-indigo-300" 
                    />
                    <datalist id="area-options">
                      {form.unitName && AREA_OPTIONS[form.unitName] 
                        ? AREA_OPTIONS[form.unitName].map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)
                        : Object.values(AREA_OPTIONS).flat().map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)
                      }
                    </datalist>
                  </div>
                </label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="space-y-1">
                  ...
                  <input value={form.functionName} onChange={(e) => {
                    onSetFormField('functionName', e.target.value);
                    if (!form.baseRoleUniversal) onSetFormField('baseRoleUniversal', e.target.value);
                  }} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 outline-none focus:border-indigo-300" />
                </label>
                <label className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Cargo-base (taxonomia)<HelpTooltip text="Cargo padrão utilizado para agrupamentos no sistema" /></span>
          ...
          <section className="space-y-4">
            <h3 className="text-[11px] font-black uppercase tracking-[0.16em] text-gray-400 border-b border-gray-100 pb-2">Bloco 3 — Report & Orientação</h3>
            <div className="grid grid-cols-2 gap-4">
              <label className="space-y-1">
              ...
              <label className="space-y-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Mentor DAI<HelpTooltip text="Agente de IA que orienta as diretrizes dessa função" /></span>
          ...
          <section className="space-y-4">
            <h3 className="text-[11px] font-black uppercase tracking-[0.16em] text-gray-400 border-b border-gray-100 pb-2">Bloco 4 — Acesso e Integração</h3>
            <div className="grid grid-cols-1 gap-4">
              {(form.entityType === 'AGENTE') && (
                <label className="space-y-1">
                  ...
                  <select value={form.usesEmail ? 'sim' : 'nao'} onChange={(e) => {
                    const usaEmail = e.target.value === 'sim';
                    onUsesEmailChange(usaEmail);
                    if (!usaEmail) onSetFormField('email', '');
                  }} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 outline-none focus:border-indigo-300"><option value="nao">Não</option><option value="sim">Sim</option></select>
                </label>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">Projeto Vinculado (Opcional)<HelpTooltip text="ID do projeto no sistema de gestão de projetos (ex: Jira, Clickup)" /></span>
          ...
          <details className="group space-y-4 rounded-2xl border border-gray-200 bg-white p-4 open:bg-gray-50/30 transition-colors">
            <summary className="flex cursor-pointer items-center justify-between font-semibold text-gray-700 outline-none marker:content-none">
              <span className="text-[11px] font-black uppercase tracking-[0.16em] text-gray-500">Motor Cognitivo & Operação Avançada</span>
            ...
            <div className="grid grid-cols-1 gap-4 pt-4 border-t border-gray-100 mt-4">
              ...
              {form.entityType === 'AGENTE' && (
                <div className="grid grid-cols-1 gap-4 mt-2 p-4 bg-indigo-50/50 rounded-xl border border-indigo-100">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-400 mb-2">Configurações Específicas de Inteligência Artificial</h4>
                  <label className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-2 rounded-xl border border-gray-200 bg-white p-3">
                      {STACK_OPTIONS.map((option) => (
                        <label key={option.value} className="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200 transition-all">
                          <input type="checkbox" checked={form.allowedStacks.includes(option.value)} onChange={() => {
                            onToggleStack(option.value);
                            // Auto-set preferred model if it's the first stack being selected, or clear if the preferred model is unchecked
                            if (!form.allowedStacks.includes(option.value) && form.allowedStacks.length === 0) {
                              onSetFormField('preferredModel', option.value);
                            } else if (form.allowedStacks.includes(option.value) && form.preferredModel === option.value) {
                               onSetFormField('preferredModel', '');
                            }
                  ...
                  <label className="space-y-1">
                    ...
                    <select value={form.preferredModel} onChange={(e) => onSetFormField('preferredModel', e.target.value as ModelProvider | '')} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 outline-none focus:border-indigo-300">
                      <option value="">Selecionar...</option>
                      {STACK_OPTIONS.filter((option) => form.allowedStacks.includes(option.value)).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                    </select>
                    {!form.preferredModel && <p className="text-[10px] font-semibold text-red-400">Selecione um modelo preferencial para a IA operar corretamente.</p>}
                  </label>
                </div>
              )}
            </div>
